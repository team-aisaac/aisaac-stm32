...
void EncoderGetCount(ENCODER* DataStruct,uint16_t GPIO_Pin,GPIO_TypeDef* GPIOx_A,uint16_t GPIO_Pin_A,GPIO_TypeDef* GPIOx_B,uint16_t GPIO_Pin_B){
	...
	if(GPIO_Pin==GPIO_Pin_A) {
		...
		if(en_stateA==GPIO_PIN_SET && en_stateB==GPIO_PIN_RESET){
			DataStruct->EncoderCount++;
			LED_RED(GPIOB,GPIO_PIN_12);
		}else if(en_stateA==GPIO_PIN_RESET && en_stateB==GPIO_PIN_SET){
			DataStruct->EncoderCount++;
			LED_RED(GPIOB,GPIO_PIN_12);
		}else if(en_stateA==GPIO_PIN_SET && en_stateB==GPIO_PIN_SET){
			DataStruct->EncoderCount--;
			LED_WHITE(GPIOB,GPIO_PIN_12);
		}else if(en_stateA==GPIO_PIN_RESET && en_stateB==GPIO_PIN_RESET){
			DataStruct->EncoderCount--;
			LED_WHITE(GPIOB,GPIO_PIN_12);
		}
		...
		en_stateA=HAL_GPIO_ReadPin(GPIOx_A, GPIO_Pin_A);
		en_stateB=HAL_GPIO_ReadPin(GPIOx_B, GPIO_Pin_B);
		if(en_stateB==GPIO_PIN_SET && en_stateA==GPIO_PIN_SET){
			DataStruct->EncoderCount++;
			LED_RED(GPIOB,GPIO_PIN_12);
		}else if(en_stateB==GPIO_PIN_RESET && en_stateA==GPIO_PIN_RESET){
			DataStruct->EncoderCount++;
			LED_RED(GPIOB,GPIO_PIN_12);
		}else if(en_stateB==GPIO_PIN_SET && en_stateA==GPIO_PIN_RESET){
			DataStruct->EncoderCount--;
			LED_WHITE(GPIOB,GPIO_PIN_12);
		}else if(en_stateB==GPIO_PIN_RESET && en_stateA==GPIO_PIN_SET){
			DataStruct->EncoderCount--;
			LED_WHITE(GPIOB,GPIO_PIN_12);
		}
	}
